...
// Owner-only. Publishes a game to the marketplace and stores its pricing/trial config.
export async function PATCH(
  ...
  const updated = await prisma.scenario.update({
    where: { id },
    data: { isPublic, priceToPlay, priceToClone, freePlayLimit, adventureSubtype },
    select: {
      id: true,
      isPublic: true,
      priceToPlay: true,
