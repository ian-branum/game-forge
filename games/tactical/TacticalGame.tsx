...
import type { GameState, Unit, Pos, Phase, ScenarioDefinition } from "./engine/types";
import {
  initGame,
  moveUnit,
  fireUnit,
...
import { playSquadFire, playLeaderFire, playMGFire } from "./engine/sounds";
import { animateTracers } from "./engine/tracers";

// ─── Hex geometry ─────────────────────────────────────────────────────────────
const HEX_SIZE = 26;
const HEX_W    = Math.round(Math.sqrt(3) * HEX_SIZE);
const HEX_H    = HEX_SIZE * 2;
const ROW_H    = Math.round(HEX_H * 0.75);
...
function TerrainIcon({ terrain, cx, cy }: { terrain: string; cx: number; cy: number }) {
  if (terrain === "woods") {
    return <text x={cx} y={cy + 4} textAnchor="middle" fontSize="13" opacity={0.55} style={{ userSelect: "none" }}>🌲</text>;
  }
  if (terrain === "building") {
    return <text x={cx} y={cy + 4} textAnchor="middle" fontSize="11" opacity={0.6} style={{ userSelect: "none" }}>🏠</text>;
  }
  if (terrain === "wall") {
    return (
      <>
  ...
  if (terrain === "rubble") {
    return <text x={cx} y={cy + 4} textAnchor="middle" fontSize="10" opacity={0.55} style={{ userSelect: "none" }}>💥</text>;
  }
  return null;
}
...
function BriefingScreen({ scenario, onStart }: { scenario: ScenarioDefinition; onStart: () => void }) {
  return (
...
export default function TacticalGame({ scenario }: TacticalGameProps) {
  ...
  const [hoveredObjLabel, setHoveredObjLabel] = useState<string | null>(null);
  const [hoveredObjBar, setHoveredObjBar]     = useState<string | null>(null);
  ...
  // ── Briefing ──────────────────────────────────────────────────────────────
  if (showBriefing) return <BriefingScreen scenario={scenario} onStart={startGame} />;
  if (!gameState) return null;

  const { units, map, objectives, turn, turnsTotal, phase, faction, log, result, activeUnit } = gameState;
      ...
      <div className="flex flex-col items-center justify-center min-h-full p-6" style={{ background: "#05071a" }}>
        <div className="text-center p-8 rounded-xl max-w-md w-full"
          ...
          <div className="flex gap-3 justify-center">
            <button onClick={restartGame} className="px-6 py-3 rounded-lg font-orbitron font-bold text-sm tracking-wider"
              style={{ background: "#ffd70022", border: "1px solid #ffd70066", color: "#ffd700" }}>
              PLAY AGAIN
            </button>
            <button onClick={handleShare} className="px-6 py-3 rounded-lg font-orbitron font-bold text-sm tracking-wider"
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="flex-1 overflow-auto p-2" style={{ position: "relative" }}>
          <canvas
            ...
            width={SVG_W}
            height={SVG_H}
            style={{ position: "absolute", top: "8px", left: "8px", width: SVG_W, height: SVG_H, pointerEvents: "none", zIndex: 10 }}
          />
          <svg
            width={SVG_W}
            height={SVG_H}
            ...
            {map.map((row, r) => row.map((tile, c) => {
              ...
              return (
                <g key={`${r}-${c}`} onClick={() => handleHexClick(r, c)} style={{ cursor: "pointer" }}>
                  <polygon points={pts} fill={fill} stroke={strokeColor} strokeWidth={strokeW} />
                  {isSelected && (
                  ...
                  {unit && unit.status !== "normal" && unit.status !== "eliminated" && (
                    <circle cx={cx + 12} cy={cy - 12} r={4} fill={STATUS_COLORS[unit.status]} stroke="#05071a" strokeWidth={1} />
                  )}
                </g>
              );
            }))}
          ...
          <div className="p-2 flex-1">
            ...
            {alliedUnits.map(u => (
              <div key={u.id}
                ...
                style={{ background: activeUnit === u.id ? "#4488ff22" : "transparent", border: activeUnit === u.id ? "1px solid #4488ff44" : "1px solid transparent", opacity: u.status === "eliminated" ? 0.4 : 1 }}>
