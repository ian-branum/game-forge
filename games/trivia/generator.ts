import { TriviaScenario, TriviaScenarioSchema } from "./schema";
...
export async function generateTriviaScenario(prompt: string): Promise<TriviaScenario> {
  ...
  const validated = TriviaScenarioSchema.parse(parsed);
  if (!validated.questions?.length) throw new Error("DeepSeek returned no questions");
  return validated;
}
